...
AtingSpectrographClient 是一个 Windows 桌面端光谱仪/成像设备控制客户端。当前代码显示它以 Qt Widgets 构建主界面，通过 TCP JSON-RPC 控制设备，通过 UDP 接收图像流，并在界面中显示 Raw16、SliceStitch16 与 Playback 图像页。客户端还支持把实时原始帧录制为 `.asrec` 文件，并从录制文件建立索引后按 FPS 回放。
...
- `src/Ui/panels/`：右侧控制面板，按功能拆分连接、相机、转镜、红外、采集、流、录制回放、日志、仪表盘。
...
- `src/Client/stream/`：UDP 图像流接收、分片重组、FPS/丢帧统计。
...
12. `src/Ui/panels/RecordPlaybackPanel.cpp`
...
8. UDP 图像包进入 `StreamClient`，`FrameAssembler` 按通道和 frame id 重组完整帧，发出 `frameReady`。
9. `MainWindow` 在收到完整帧时，如果录制开启，先把原始帧数据交给 `FrameRecorder` 写入 `.asrec`。
10. `MainWindow` 将 Mono8/Mono16 数据转换为 8-bit `QImage`，显示到 Raw16 或 SliceStitch16 图像视图，并更新 FPS/帧数/丢帧统计。
11. `RecordPlaybackPanel` 选择录制文件后由 `FramePlaybackController` 异步扫描索引；开始回放会自动切到 Playback 图像页。
...
- 录制写入完整原始帧数据，保留通道、宽高、像素格式、帧序号、相对时间戳、payload 字节数和 CRC32。
